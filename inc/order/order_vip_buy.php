...
<div class="wrap">
	<h2><i class="fa fa-diamond"></i>会员销售统计</h2>
...
class RiPlus_List_Table extends WP_List_Table
{
    ...
    public function column_default( $item, $column_name )
    {
        switch ( $column_name ) {
            ...
            case 'post_id':
            	...
            	$vip_opt  = _get_site_vip_options();
            	return '<span class="badge badge-light"><i class="fa fa-diamond"></i> 购买 / '.$vip_opt[$order_info["vip_type"]]['name'].'会员</span>';
            case 'order_price':
    ...
    private function table_data($per_page = 5, $page_number = 1 )
    {
		...
		$sql = "SELECT * FROM $order_table_name WHERE post_id=0 AND order_type=2";
		//根据用户查询
		if ( ! empty( $_REQUEST['s'] ) ) {
			$user_id = 0;
    ...
    private function table_data_count() {
        ...
        $sql = "SELECT COUNT(*) FROM $order_table_name WHERE post_id=0 AND order_type=2";
		//根据用户查询
		if ( ! empty( $_REQUEST['s'] ) ) {
			$user_id = 0;
			if (is_numeric($_REQUEST['s'])) {
